...
from plugin_main.widgets.users_treewidget import UsersTreeWidget

class UsersDialog_with_tree_table(QDialog):
    """사용자 목록을 표시하는 다이얼로그
        kwargs:
            app_ID:int : 앱 권한 ID
            ...
            추가됨: 
            user_attr_name:str : default는 'user'임. pre_selected_ids 의 사용자 ID 속성 이름
            view_type:str : default는 'select'임. 'select' 또는 'preview'
    """
    users_changed = pyqtSignal(dict)
    
    def __init__(self, parent=None, app_ID:int=None, pre_selected_ids:list[dict]=None, 
                 all_users=None, api_url=None, on_complete_channelName=None, view_type='select', **kwargs):
        ...
        self.user_attr_name = kwargs.get('user_attr_name', 'user')
 
        self.map_id_to_selected_ids = { selected.get(self.user_attr_name): selected for selected in self.pre_selected_ids }
        ...
        self.view_type = view_type #kwargs.get('view_type', 'select') #### 'select
    ...
    def setup_ui(self):
        ...
        self.tree_widget = UsersTreeWidget(self, self.pre_selected_ids, self.all_users, view_type=self.view_type)
        self.tree_widget.selected_changed.connect(self.slot_selected_changed_from_tree)
        layout.addWidget(self.tree_widget)

        self.table_widget = UsersTableWidget(self, self.pre_selected_ids, self.all_users, view_type=self.view_type)
    ...
    def slot_selected_changed_from_tree(self, userObj:dict):
        ...
        if hasattr( self, 'table_widget') and hasattr( self.table_widget, 'sync_with_tree'):
    ...
    def slot_selected_changed_from_table(self, userObj:dict):
        ...
        if hasattr( self, 'tree_widget') and hasattr( self.tree_widget, 'simulate_click_user_by_id'):
    ...
    def on_ok_button_clicked(self):
        ...
        if not self.confirm_save(selected_users, added_users, deleted_ids):
            return
        # 버튼 비활성화
