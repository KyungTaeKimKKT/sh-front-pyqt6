...
class Wid_Table_for_app사용자(QWidget , Handle_Table_Menu):
	"""
...
###############################################################################################
	...
	def slot_signal_model_data_changed(self, msg:dict):
		...
		user_pks:list = self.app_Dict.get('user_pks')
		...
		_isOK, _json = APP.API.Send ( url=self.url_APP설정, dataObj=self.app_Dict, sendData={'user_pks': user_pks} )
		if _isOK:
			...
			logger.error(f"사용자 설정 오류: {_json}")
