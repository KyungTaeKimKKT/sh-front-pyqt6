...
class Wid_생산지시서_SPG(QWidget, Qwidget_Utils ):
	...
	def user_defined_UI(self):
		self.ui.PB_import_Process.clicked.connect( self.slot_import_Process )
		return 

	def slot_import_Process(self):

		ic (self.dataObj )
		ic('slot_import_Process')	
		#### 😀 Process_fks : 즉, HTM_Table
		_isOk, self.작업지침_obj = APP.API.getObj(INFO.URL_작업지침_이력조회, id= self.dataObj.get('작업지침_fk', -1) )
		if _isOk:
			process_fks =  self.작업지침_obj.get('process_fks', []) 
		else:
	...
	def triggerConnect(self):
		return
		self.ui.PB_import_Process.clicked.connect( lambda : ic( 'pbImportClicked') )
		self.ui.PB_import_Process.clicked.connect( self.slot_import_Process )
