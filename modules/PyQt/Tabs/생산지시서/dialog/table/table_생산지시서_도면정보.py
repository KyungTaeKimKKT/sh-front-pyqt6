	...
	def calculateSum(self):
		...
		for colNo, headerName in enumerate(self.header):
			if '수량2' in headerName:
				# 해당 컬럼의 모든 유효한 숫자값을 리스트로 수집
				values = [row[colNo] for row in self._calucrated_data if isinstance(row[colNo], (int, float))]
				
				# float 값이 하나라도 있는지 확인
				has_float = any(isinstance(v, float) for v in values)
				
				if has_float:
					# float이 있으면 합계를 구한 후 올림 처리
					value_sum = math.ceil(sum(values))
				else:
					# 모두 정수면 그냥 더하기
...
class Delegate_생산지시서_도면정보(My_Table_Delegate):
	...
	def user_defined_creatorEditor_설정(self, widget:object, **kwargs) -> object:		
		index : QModelIndex = kwargs.get('index', None)

		if index is not None:
			rowNo, colNo = index.row(), index.column()
			if rowNo <4 :
				if rowNo == 0 or rowNo == 1:
					widget = QTextEdit( kwargs['parent'])
					widget.setMinimumHeight(48)
				elif rowNo == 2 or rowNo == 3:
					widget = QLineEdit( kwargs['parent'])
				...
				if isinstance(widget, QDateEdit):
					...
					widget.setTimeSpec(QtCore.Qt.TimeSpec.LocalTime)
					widget.setDate(QDate.currentDate().addMonths(1))  # 현재 날짜에서 1달 후로 설정
					return widget

				return super().user_defined_creatorEditor_설정(widget, **kwargs)

	
	def setModelData(self, editor, model:QAbstractItemModel, index:QModelIndex):
		if isinstance(editor, QDateEdit):
			value = editor.date().toString("yyyy-MM-dd")  # 날짜를 문자열로 변환
