...
from modules.PyQt.Tabs.생산지시서.dialog.list_edit.dlg_치수선택 import 치수선택다이얼로그
...
class TableModel_생산지시서_의장(My_TableModel):
	...
	def calculateSum_byHeadName(self, headerName:str, **kwargs):
		""" Override : headerName에 대한 합계 ==> 단, 적용에 'jamb'가 포함되지 않은 것만 합계"""
		...
		value_sum = sum(row[colNo] for row in self._calucrated_data if isinstance(row[colNo], (int, float)) and 'jamb' not in row[self.table_header.index('적용')].lower() )
...
class Delegate_생산지시서_의장(My_Table_Delegate):
	...
	def user_defined_creatorEditor_설정(self, widget:object, **kwargs) -> object:
		match kwargs['key']:
			...
			case '치수'|'출하처':
				# widget = 치수선택위젯(kwargs['parent'], 치수_list=self.kwargs['치수_LIST'])
				# parent가 TableView인지 확인하고 viewport 위치 계산
				...
				if kwargs['key'] == '치수':
					dialog = 치수선택다이얼로그(kwargs['parent'], 치수_list=self.kwargs.get('치수_LIST', []), pos=pos)
				elif kwargs['key'] == '출하처':
					dialog = 판금처선택다이얼로그(kwargs['parent'], 판금처_list=self.kwargs.get('판금처_LIST', []), pos=pos)
	...
	def setModelData(self, editor, model:QAbstractItemModel, index:QModelIndex):
		if isinstance(editor, 치수선택다이얼로그|판금처선택다이얼로그):
