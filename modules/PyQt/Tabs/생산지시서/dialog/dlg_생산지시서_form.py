...
from modules.PyQt.Tabs.생산지시서.dialog.widget.Wid_생산지시서_main import Wid_생산지시서_main
from modules.PyQt.Tabs.생산지시서.dialog.widget.Wid_생산지시서_SPG  import Wid_생산지시서_SPG
from modules.PyQt.Tabs.생산지시서.dialog.widget.Wid_생산지시서_SPG_empty import Wid_생산지시서_SPG_Empty
...
class Dialog_생산지시서_Form( QDialog, Qwidget_Utils):
	...
	def __init__(self,  parent,  **kwargs  ):
		...
		self.mainName = '생산지시서'
		self.tabs_config = {self.mainName: 'Wid_생산지시서_main',
					  		'SPG' : 'Wid_생산지시서_SPG' ,}
		self.tabs_Elms = {}
		self.tabs_made_List = []
	...
	def init_UI_tabs(self) -> None:		
		...
		for name, className in self.tabs_config.items():
			...
			if name == self.mainName:
				...
				if 'spg_fks' in self.dataObj and ( spg_fks := self.dataObj['spg_fks'] ):
					for spg_fk in spg_fks:
					...
					wid = eval(f"{className}(self, **self.kwargs, spg_fk= -1 )")
					# wid:Wid_생지_main|Wid_생지_spg
					# setattr( self, name, wid )
					self.tabs_Elms[name] = wid
					self.tabWidgets.addTab(wid, name)
	...
	def TriggerConnect(self):
		...
		if isinstance( wid_mainTab, Wid_생산지시서_main ):
	...
	def slot_createTab(self, **kwargs ):
		ic( kwargs )
		self.tab_count += 1
		wid =  Wid_생산지시서_SPG_Empty(self, **kwargs )
	...
	def slot_signal_save (self, sendData:dict):
		...
		if self.is_update_By_Jakji:
			sendData['작업지침_fk'] = self.작업지침_obj.get('id')
		ic ( sendData )
