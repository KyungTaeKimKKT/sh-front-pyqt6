...
class TableView_생산지시서_완료(My_TableView):
	def __init__(self, parent, **kwargs):
		super().__init__(parent, **kwargs)

class TableModel_생산지시서_완료(My_TableModel):
	def __init__(self, parent, **kwargs):
		super().__init__(parent, **kwargs)

class Delegate_생산지시서_완료(My_Table_Delegate):
	def __init__(self, parent, **kwargs):
		super().__init__(parent, **kwargs)
...
class Wid_Table_for_생산지시서_완료(QWidget , Handle_Table_Menu):
	"""
	...
	def __init__(self, parent,  **kwargs ):
		super().__init__( parent, **kwargs )
		self.tableView:  TableView_생산지시서_완료
		self.table_model : TableModel_생산지시서_완료
		self.delegate : Delegate_생산지시서_완료
	...
	def slot_Sample_completed( self, dlg:QDialog, compledtedDict:dict, msg:dict) :
		URL_파일_m2m = INFO.URL_생산지시서_완료FILE
		futures = []
	...
	def slot_select_row(self, wid:QWidget, 생산지시서_완료_apiDict:dict, EL_한국정보_ID:int) :
		""" apiDict : Elevator 한국정보 Model data로 \n
			apiDict.get('id') 로 fk 사용
		"""
		_is_ok, _json = APP.API.Send( self.url, 생산지시서_완료_apiDict, {'현장명_fk': EL_한국정보_ID})
		if _is_ok:
			wid.close()
			self.signal_refresh.emit()
		else:
	...
	def menu__new_row(self, msg:dict) -> None:
		"""
			copy msg.get('row') 하여 insert 함, 생산지시서_완료은 일자만 복사하여 유지		
		"""
		row :int = msg.get('row')
		self.tableView : TableView_생산지시서_완료
	...
	def menu__upgrade_row(self, msg:dict) ->None:
		"""
			copy msg.get('row') 하여 insert 함, 생산지시서_완료은 버젼만 default 0.01 up함		
		"""
		row :int = msg.get('row')
		self.tableView : TableView_생산지시서_완료
		model_datas:list[list] = self.tableView.model()._data
	...
	def _get_m2mField_info (self, msg:dict) ->tuple[str,str] :
		...
		match self._get_headerName(colNo):
			...
			case '완료파일수':
				m2mField = '완료file_fks'
				URL_파일_m2m  = '/생산지시서_완료/완료file-viewSet/'
				URL_파일_m2m  = getattr( self, f"URL_{m2mField}")
			case _:
				URL_파일_m2m = ''
				m2mField = ''
