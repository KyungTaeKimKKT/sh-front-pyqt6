...
class TableView_영업수주_Summary(My_TableView):
...
class TableModel_영업수주_Summary(My_TableModel):
	def __init__(self, parent, **kwargs):
		super().__init__(parent, **kwargs)
...
class Delegate_영업수주_Summary(My_Table_Delegate):
	def __init__(self, parent, **kwargs):
		super().__init__(parent, **kwargs)
		self.kwargs = kwargs
...
TABLE_NAME = '영업수주_Summary'

HOVER_LIST = []


class Wid_Table_for_영업수주_Summary(QWidget , Handle_Table_Menu):
	"""
	...
	def __init__(self, parent,  **kwargs ):
		super().__init__( parent, **kwargs )
		self.tableView:  TableView_영업수주_Summary
		self.table_model : TableModel_영업수주_Summary
		self.delegate : Delegate_영업수주_Summary
	...
	def slot_Sample_completed( self, dlg:QDialog, compledtedDict:dict, msg:dict) :
		URL_파일_m2m = INFO.URL_영업수주_SummaryFILE
		futures = []
	...
	def slot_select_row(self, wid:QWidget, 영업수주_Summary_apiDict:dict, EL_한국정보_ID:int) :
		""" apiDict : Elevator 한국정보 Model data로 \n
			apiDict.get('id') 로 fk 사용
		"""
		_is_ok, _json = APP.API.Send( self.url, 영업수주_Summary_apiDict, {'현장명_fk': EL_한국정보_ID})
		if _is_ok:
			wid.close()
			self.signal_refresh.emit()
		else:
	...
	def menu__new_row(self, msg:dict) -> None:
		"""
			copy msg.get('row') 하여 insert 함, 영업수주_Summary은 일자만 복사하여 유지		
		"""
		row :int = msg.get('row')
		self.tableView : TableView_영업수주_Summary
	...
	def menu__upgrade_row(self, msg:dict) ->None:
		"""
			copy msg.get('row') 하여 insert 함, 영업수주_Summary은 버젼만 default 0.01 up함		
		"""
		row :int = msg.get('row')
		self.tableView : TableView_영업수주_Summary
		model_datas:list[list] = self.tableView.model()._data
