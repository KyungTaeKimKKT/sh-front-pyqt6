...
class GanttScene(QGraphicsScene, LazyParentAttrMixin):
    ...
    def draw_chart(self):
        ...
        for d in self.data:
            if d.get("등록일") :
                if isinstance(d["등록일"], str):
                    d["등록일"] = datetime.strptime(d["등록일"].split('T')[0], "%Y-%m-%d").date()
            else:
                d["등록일"] = datetime.today().date()
            if d.get("완료요청일"):
                if isinstance(d["완료요청일"], str):
                    d["완료요청일"] = datetime.strptime(d["완료요청일"].split('T')[0], "%Y-%m-%d").date()
            else:
                d["완료요청일"] = d["등록일"] + timedelta(days=7)

            if d.get("완료일"):
                if isinstance(d["완료일"], str):
...
class GanttMainWidget(QWidget):
    ...
    def _init_by_data(self, data:list[dict]):
        ...
        dates = [
            datetime.fromisoformat(item['등록일'])
            for item in data
            if item.get('등록일')
        ]
