...
class TableView_품질경영_CS_Activity_이력조회(TableView_품질경영_CS관리):
    set_row_span_list = [
        ('el수량', ['id']),
        ('현장명', ['id']),
        ('운행층수', ['id']),
...
class TableModel_품질경영_CS_Activity_이력조회(TableModel_품질경영_CS관리):

    def _role_display( self, row:int, col:int) -> str:
        """표시 데이터 반환 : 현재 base는 없으면 오류 발생토록 했음."""
        if not self._data  : return ''
        
        attribute_name = self.get_field_name_by_column_no(col)
    ...
    def _role_textAlignment( self, row:int, col:int) -> Qt.Alignment:
        """텍스트 정렬 반환"""
        if col == self.get_column_No_by_field_name('불만요청사항') or col == self.get_column_No_by_field_name('활동현황'):
...
class TableDelegate_품질경영_CS_Activity_이력조회(TableDelegate_품질경영_CS관리):
    pass
...
class Wid_table_품질경영_CS_Activity_이력조회( Wid_table_품질경영_CS관리 ):
    ...
    def setup_table(self):
        self.model = TableModel_품질경영_CS_Activity_이력조회(self)
        self.view = TableView_품질경영_CS_Activity_이력조회(self)
        self.delegate = TableDelegate_품질경영_CS_Activity_이력조회(self.view)
