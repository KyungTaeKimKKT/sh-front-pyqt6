...
from modules.PyQt.Tabs.품질경영.chart.gantt_chart import GanttMainWidget
...
class CS관리__for_stacked_Table( Base_Stacked_Table ):
	default_view_name = DEFAULT_VIEW

	def init_ui(self):
		self.mapping_tables = {
			'chart': GanttMainWidget(self),
			'테이블': self.create_table(),
		}
	...
	def apply_api_datas(self, datas:Optional[list[dict]]=None):
		...
		logger.debug(f"{self.__class__.__name__} : apply_api_datas: self.default_view_name : {self.default_view_name}")
		if datas:
			if not hasattr(self, 'default_view_name'):
				self.default_view_name = getattr( type(self), 'default_view_name', None)
			logger.debug(f"{self.__class__.__name__} : apply_api_datas: self.default_view_name : {self.default_view_name}")
			logger.debug(f"{self.__class__.__name__} : apply_api_datas: self.mapping_widgets : {self.mapping_widgets}")
			#### GanttMainWidget 초기화
			gantt_chart:GanttMainWidget = self.mapping_widgets['chart']
			gantt_chart._init_by_data(datas)

			if hasattr(self, 'default_view_name') and self.default_view_name in self.mapping_widgets:
				self.setCurrentWidget(self.default_view_name)
			else:
				raise ValueError(f"default_view is not found in {self.__class__.__name__}")



class CS관리__for_Tab( BaseTab ):
	...
	def create_ui(self):
		...
		self.stacked_table = CS관리__for_stacked_Table(self)
	...
	def custom_ui_add_combo_view(self):
		### view 선택 combo 삽입		
		...
		self.combo_view.addItems(['chart', '테이블'])
		self.combo_view.setCurrentText(self.default_view_name)
	...
	def custom_ui(self):
		...
		self.PB_Map_View.setDisabled(True)
	...
	def subscribe_gbus(self):
		...
		#### gantt chart 용
		self.event_bus.subscribe(f"{self.table_name}:request_open_form", self.on_request_open_form)
	...
	def on_selected_rows(self, selected_rows:list[dict]):
		...
		if hasattr(self, 'pb_claim_open'):
			self.pb_claim_open.setEnabled(bool(selected_rows))
		if hasattr(self, 'pb_claim_close'):
			self.pb_claim_close.setEnabled(bool(selected_rows))
		if hasattr(self, 'PB_Map_View'):
			self.PB_Map_View.setEnabled(bool(selected_rows))

	def on_claim_open(self):
		""" model에서  subscribe 되어 있음 """
	...
	def on_request_open_form(self, site_data:dict):
		self.selected_rows = [site_data]
		self.on_claim_project()
	...
	def on_claim_project(self):
		...
		try:
			if hasattr(self, 'table_name') and self.table_name:
				...
				raise ValueError(f"table_name is not found in {self.__class__.__name__}")
