...
class BaseTab_V3(QWidget, Ui_Base_Tab_V2):
    """모든 탭의 v3 버젼 기본 골격"""
    ...
    def slot_fetch_finished(self, msg) -> tuple[bool, dict, list[dict]]:
        ...
        is_ok = copyed_msg.get('is_ok', False)
        if is_ok:
            ...
            is_ok, pagination, api_datas = False, {}, []
            msg = copyed_msg.get('results', {})
            _text = f""" 
            서버 조회 오류 입니다.<br>
            {json.dumps(msg if msg else copyed_msg, ensure_ascii=False, indent=4)}<br>
