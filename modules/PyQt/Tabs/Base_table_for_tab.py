...
class Base_Wid_Table(QtWidgets.QWidget):
	def __init__(self, parent:QtWidgets.QMainWindow=None, **kwargs):
	...
	def run(self):
		...
		if self.get_api_and_model_table_generate():
			Utils.wrtie_app_DB_data_to_json(self.app_DB_data, '생산지시_appData.json' )			
	...
	def run_by_parent(self, app_DB_data : list) -> None:
		""" self.app_DB_data를 parent에서 write 하고, 이 method실행"""
	...
	def my_span_checker(self, action:str, my_data, table, colNo:int):
		match action:
			case 'set':
				# rows =len(my_data)
				row_span_cnt = 0
	...
	def _get_Obj_from_app_DB_data(self, msg:dict) -> dict:
		if msg and (rowNo:=msg.get('data').get('row') ) is not None:
			return self.app_DB_data[rowNo]
		else:
			return {}