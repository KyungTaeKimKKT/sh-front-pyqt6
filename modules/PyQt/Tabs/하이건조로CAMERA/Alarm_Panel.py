...
class Alarm_Panel__for_Tab(BaseTab_V3):
        ...
        try:
            ...
            self.pb_refresh.clicked.connect(
                lambda: self.event_bus.publish (
                     f"AppID:{self.id}_{GBus.SEARCH_REQUESTED}",
    ...
    def check_ws(self):
        ...
        #     self.ws_url_name = self.lazy_attrs.get('ws_url_name', "app_broadcast/hi_rtsp_led_anaylizer")
        #     if not self.ws_url_name:
        #         raise ValueError(f"ws_url_name 이 없습니다. {self.ws_url_name}")
        #     self.set_ws_url(self.ws_url_name)

        self.ws_url =  "app_broadcast/hi_rtsp_led_anaylizer/"
        ...
        if self.ws_url not in INFO.WS_TASKS:
            ...
            ws_manager.add(self.ws_url, connect_msg=connect_msg, message_slot_func=self.on_ws_data_via_local)
    ...
    def on_ws_data_via_local(self, url:str, data:dict):
        print (f"on_ws_data_via_local: {url} {data.keys()}")
        self.on_ws_data_changed(data)
