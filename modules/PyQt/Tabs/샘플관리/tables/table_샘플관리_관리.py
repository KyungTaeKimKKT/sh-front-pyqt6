...
# import user module
# from modules.PyQt.Tabs.차량관리.dialog.dlg_user관리 import Dialog_차량관리_User관리
...
from modules.PyQt.Tabs.샘플관리.dialog.dlg_샘플의뢰_form import Dialog_샘플의뢰_Form
from modules.PyQt.Tabs.샘플관리.dialog.dlg_샘플완료_form import Dialog_샘플완료
from modules.PyQt.Tabs.샘플관리.dialog.dlg_샘플결과_view import Dialog_ImageViewer
...
class TableView_샘플관리_관리(My_TableView):
	def __init__(self, parent, **kwargs):
		super().__init__(parent, **kwargs)

class TableModel_샘플관리_관리(My_TableModel):
	def __init__(self, parent, **kwargs):
		super().__init__(parent, **kwargs)

class Delegate_샘플관리_관리(My_Table_Delegate):
	def __init__(self, parent, **kwargs):
		super().__init__(parent, **kwargs)
...
class Wid_Table_for_샘플관리_관리(QWidget , Handle_Table_Menu):
	"""
	...
	def __init__(self, parent,  **kwargs ):
		super().__init__( parent, **kwargs )
		self.tableView:  TableView_샘플관리_관리
		self.table_model : TableModel_샘플관리_관리
		self.delegate : Delegate_샘플관리_관리
	...
	def menu__form_new_row(self, msg:dict) -> None:
		...
		newObj = {'id':-1, '요청일': today, '요청자':INFO.USERNAME, '납기일' :today+timedelta(days=2),  }
		dlg = Dialog_샘플의뢰_Form(self, url=self.url,  dataObj = newObj )
	...
	def menu__form_update_row(self, msg:dict) -> None:
		...
		dlg = Dialog_샘플의뢰_Form(self, url=self.url,  dataObj = dataObj )		
		dlg.signal_refresh.connect ( lambda: self.signal_refresh.emit() )

	def menu__form_view_row(self, msg:dict) -> None:
		dataObj = self.api_data[msg.get('row')]
		dlg = Dialog_샘플의뢰_Form(self, url=self.url,  dataObj = dataObj , is_Edit=False )
	...
	def slot_signal_model_data_changed(self, msg:dict) -> None:
		...
		if headerName == 'is_완료' and msg.get('value'):
			dlg = Dialog_샘플완료(self)
			dlg.completed.connect (lambda copmpltedDict, dlg=dlg, msg=msg: self.slot_Sample_completed(dlg, copmpltedDict, msg ))
	...
	def slot_Sample_completed( self, dlg:QDialog, compledtedDict:dict, msg:dict) :
		URL_파일_m2m = INFO.URL_샘플관리_관리FILE
		futures = []
	...
	def slot_select_row(self, wid:QWidget, 샘플관리_관리_apiDict:dict, EL_한국정보_ID:int) :
		""" apiDict : Elevator 한국정보 Model data로 \n
			apiDict.get('id') 로 fk 사용
		"""
		_is_ok, _json = APP.API.Send( self.url, 샘플관리_관리_apiDict, {'현장명_fk': EL_한국정보_ID})
		if _is_ok:
			wid.close()
			self.signal_refresh.emit()
		else:
	...
	def menu__new_row(self, msg:dict) -> None:
		"""
			copy msg.get('row') 하여 insert 함, 샘플관리_관리은 일자만 복사하여 유지		
		"""
		row :int = msg.get('row')
		self.tableView : TableView_샘플관리_관리
	...
	def menu__upgrade_row(self, msg:dict) ->None:
		"""
			copy msg.get('row') 하여 insert 함, 샘플관리_관리은 버젼만 default 0.01 up함		
		"""
		row :int = msg.get('row')
		self.tableView : TableView_샘플관리_관리
		model_datas:list[list] = self.tableView.model()._data
