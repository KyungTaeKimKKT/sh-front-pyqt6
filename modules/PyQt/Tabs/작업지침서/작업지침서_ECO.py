...
class ECO__for_Tab( QWidget, Utils_QWidget):
	def __init__(self, parent:QtWidgets.QMainWindow,   **kwargs ):
		...
		self.ui.label_target.setText( "작업지침서를 ECO 작성합니다. (좌측 이력조회로 기발행된 작업지침서를 선택하여 작성 )")

		### 😀 이력조회를 위한 button생성
		self.PB_History = QPushButton( '이력조회' )
		self.ui.horizontalLayout.addWidget(self.PB_History)
		self.PB_History.clicked.connect ( self.slot_PB_History)
		################################
	...
	@pyqtSlot()
	def slot_PB_History(self):
		from modules.PyQt.Tabs.작업지침서.작업지침서_이력조회 import 이력조회__for_Tab

		dlg = QDialog(self)
		vLayout = QVBoxLayout()
		hlayout = QHBoxLayout()
		label = QLabel("ECO(작업지침서 변경)를 하고자 하는 작업지침서를 선택하시기 바랍니다.")
		...
		dlg.setWindowTitle( "ECO 대상 작업지침서 검색")
		dlg.show()

		# wid.ui.Wid_Table._get_selected_row_Dict()
		PB_Select.clicked.connect ( lambda : self.slot_eco_target_selected ( dlg, wid.ui.Wid_Table ) )

	@pyqtSlot(QDialog, QWidget)
	def slot_eco_target_selected (self, dlg:QDialog , wid_Table:QWidget ):
		...
		if ( selectObj := wid_Table._get_selected_row_Dict() ):
			...
			newObj.update ( { '작성일': today, '작성자':INFO.USERNAME, '납기일' :today+timedelta(days=30), 'Rev':selectObj['Rev']+1 , '변경사유_내용':''} )

			dlg =Dialog_작업지침서_Form(self, url=self.url,  dataObj = newObj , is_ECO=True )
			dlg.setWindowTitle (f"ECO작성 : {newObj['제목']} - Rev : {newObj['Rev']}")
			dlg.signal_refresh.connect ( lambda:self.slot_search_for(self.param) )
		else:
			Utils.generate_QMsg_critical(self, title="ECO 대상 작업지침서 오류",  text='선택된 ECO 대상 작업지침서가 없읍니다.')