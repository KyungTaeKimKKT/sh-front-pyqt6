...
class TableView_작업지침서_관리(My_TableView):
...
class TableModel_작업지침서_관리(My_TableModel):
	def __init__(self, parent, **kwargs):
		super().__init__(parent, **kwargs)

class Delegate_작업지침서_관리(My_Table_Delegate):
	def __init__(self, parent, **kwargs):
		super().__init__(parent, **kwargs)
...
class Wid_Table_for_작업지침서_관리(QWidget , Handle_Table_Menu):
	"""
	...
	def __init__(self, parent,  **kwargs ):
		super().__init__( parent, **kwargs )
		self.tableView:  TableView_작업지침서_관리
		self.table_model : TableModel_작업지침서_관리
		self.delegate : Delegate_작업지침서_관리
	...
	def menu__form_new_row(self, msg:dict) -> None:
		...
		dlg =Dialog_작업지침서_Form(self, url=self.url,  dataObj = newObj )
	...
	def menu__form_update_row(self, msg:dict) -> None:
		...
		dlg =Dialog_작업지침서_Form(self, url=self.url,  dataObj = dataObj )		
		dlg.signal_refresh.connect ( lambda: self.signal_refresh.emit() )

	def menu__form_view_row(self, msg:dict) -> None:
		dataObj = self.api_data[msg.get('row')]
		dlg =Dialog_작업지침서_Form(self, url=self.url,  dataObj = dataObj , is_Edit=False )
	...
	def menu__eco_row(self , msg:dict ):
		...
		dlg =Dialog_작업지침서_Form(self, url=self.url,  dataObj = newObj , is_ECO=True )
		dlg.signal_refresh.connect ( lambda: self.signal_refresh.emit() )
	...
	def slot_signal_model_data_changed(self, msg:dict) -> None:
		...
		if headerName == 'is_배포' and msg.get('value'):
			appDict = self.api_data[rowNo]
			text = '\n\n'
			for key in ['고객사', '제목', 'Proj_No', 'Rev', '납기일']:
				if key == 'Rev':
					text += f" {key} : {appDict[key]}({'초도발행' if appDict[key] == 1 else 'ECO'}) \n"
				else :
					text += f" {key} : {appDict[key]}\n "

			dlg_res_button =  Utils.generate_QMsg_question(self, title="배포확인", text= text + '\n\n 배포를 하시겠읍니까? \n')
			if dlg_res_button == QMessageBox.StandardButton.Ok :
				is_ok, _json = APP.API.Send ( self.url, dataObj, send_data )
				if is_ok:
					self.signal_refresh.emit()
				else:
					Utils.generate_QMsg_critical(self)
	...
	def slot_Sample_completed( self, dlg:QDialog, compledtedDict:dict, msg:dict) :
		URL_파일_m2m = INFO.URL_작업지침서_관리FILE
		futures = []
	...
	def slot_select_row(self, wid:QWidget, 작업지침서_관리_apiDict:dict, EL_한국정보_ID:int) :
		""" apiDict : Elevator 한국정보 Model data로 \n
			apiDict.get('id') 로 fk 사용
		"""
		_is_ok, _json = APP.API.Send( self.url, 작업지침서_관리_apiDict, {'현장명_fk': EL_한국정보_ID})
		if _is_ok:
			wid.close()
			self.signal_refresh.emit()
		else:
	...
	def menu__new_row(self, msg:dict) -> None:
		"""
			copy msg.get('row') 하여 insert 함, 작업지침서_관리은 일자만 복사하여 유지		
		"""
		row :int = msg.get('row')
		self.tableView : TableView_작업지침서_관리
	...
	def menu__upgrade_row(self, msg:dict) ->None:
		"""
			copy msg.get('row') 하여 insert 함, 작업지침서_관리은 버젼만 default 0.01 up함		
		"""
		row :int = msg.get('row')
		self.tableView : TableView_작업지침서_관리
		model_datas:list[list] = self.tableView.model()._data
