...
class TableView_SCM_제품_관리(My_TableView):
...
class TableModel_SCM_제품_관리(My_TableModel):
	def __init__(self, parent, **kwargs):
		super().__init__(parent, **kwargs)
...
class Delegate_SCM_제품_관리(My_Table_Delegate):
	def __init__(self, parent, **kwargs):
		super().__init__(parent, **kwargs)
		self.kwargs = kwargs
...
TABLE_NAME = 'SCM_제품_관리'

HOVER_LIST = []


class Wid_Table_for_SCM_제품_관리(QWidget , Handle_Table_Menu):
	"""
	...
	def __init__(self, parent,  **kwargs ):
		super().__init__( parent, **kwargs )
		self.tableView:  TableView_SCM_제품_관리
		self.table_model : TableModel_SCM_제품_관리
		self.delegate : Delegate_SCM_제품_관리
	...
	def slot_Sample_completed( self, dlg:QDialog, compledtedDict:dict, msg:dict) :
		URL_파일_m2m = INFO.URL_SCM_제품_관리FILE
		futures = []
	...
	def slot_select_row(self, wid:QWidget, SCM_제품_관리_apiDict:dict, EL_한국정보_ID:int) :
		""" apiDict : Elevator 한국정보 Model data로 \n
			apiDict.get('id') 로 fk 사용
		"""
		_is_ok, _json = APP.API.Send( self.url, SCM_제품_관리_apiDict, {'현장명_fk': EL_한국정보_ID})
		if _is_ok:
			wid.close()
			self.signal_refresh.emit()
		else:
	...
	def menu__new_row(self, msg:dict) -> None:
		"""
			copy msg.get('row') 하여 insert 함, SCM_제품_관리은 일자만 복사하여 유지		
		"""
		row :int = msg.get('row')
		self.tableView : TableView_SCM_제품_관리
	...
	def menu__upgrade_row(self, msg:dict) ->None:
		"""
			copy msg.get('row') 하여 insert 함, SCM_제품_관리은 버젼만 default 0.01 up함		
		"""
		row :int = msg.get('row')
		self.tableView : TableView_SCM_제품_관리
		model_datas:list[list] = self.tableView.model()._data
