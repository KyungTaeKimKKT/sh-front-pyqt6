...
### ui import
from modules.PyQt.Tabs.공지및요청사항.ui.Ui_search_compenet_요청사항 import Ui_Search
...
class Wid_Search_for(QWidget, Qwidget_Utils):
	...
	def init_InputDict(self) ->None:
		self.inputDict = {
			...
			'내용' : self.ui.lineEdit_Naeyong,
			'요청등록일' : self.ui.dateEdit_Sijak,
			# '종료일' : self.ui.dateEdit_Jonryo,
			'is_완료' : self.ui.checkBox_isWanryo,
	...
	def _get_api_query_params(self, search_dict:dict) -> str:
		option_enabled = self.ui.checkBox_Date.isChecked()
		...
		for (key, value ) in search_dict.items():
			...
			if not option_enabled and key in ['요청등록일']: continue				
