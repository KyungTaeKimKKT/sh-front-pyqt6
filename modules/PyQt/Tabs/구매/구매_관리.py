...
# from modules.PyQt.Tabs.구매.dialog.dlg_구매_form import Dialog_구매_Form
### 😀😀 user : ui...
from modules.PyQt.Tabs.구매.ui.Ui_tab_구매_관리 import Ui_Tab_App as Ui_Tab
###################
...
class 관리__for_Tab( QWidget, Utils_QWidget):
	...
	def slot_Jakji_search(self):
		...
		vLayout = QVBoxLayout()
		hlayout = QHBoxLayout()
		label = QLabel("구매를 발행 하고자 하는 작업지침서를 선택하시기 바랍니다.")
		...
		dlg.setWindowTitle( "구매 대상 작업지침서 검색")
		dlg.show()
	...
	def slot_Prod_Instruction_target_selected (self, dlg:QDialog , wid_Table:QWidget ):
		...
		if ( selectObj := wid_Table._get_selected_row_Dict() ):
			...
			dlg =Dialog_구매_Form(self, url=self.url,  dataObj = newObj , is_update_By_Jakji=True , 작업지침_obj = selectObj )
			dlg.setWindowTitle (f"구매 작성 : {selectObj['제목']} - Rev : {selectObj['Rev']} 작업지침서")

			dlg.signal_refresh.connect ( lambda:self.slot_search_for(self.param) )
		else:
			Utils.generate_QMsg_critical(self, title="구매 대상 작업지침서 오류",  text='선택된 작업지침서가 없읍니다.')
