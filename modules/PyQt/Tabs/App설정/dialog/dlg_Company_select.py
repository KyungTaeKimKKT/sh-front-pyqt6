...
class Dlg_Company_Select(QDialog):
    def __init__(self, parent=None, Company_list=None, pos=None):
        super().__init__(parent)
        self.Company_list = Company_list or []
        self.setWindowTitle("Company 선택")
        self.selected_value = ""
    ...
    def setup_ui(self):
        ...
        # 주의문구 추가
        주의문구 = QLabel("※ Company 등록/삭제는 관리자에게 문의 바람")
        주의문구.setStyleSheet("background-color:black;color: yellow;font-weight:bold;")
        layout.addWidget(주의문구)

        # Company선택위젯 추가
        self.Company위젯 = Company선택위젯(self, self.Company_list)
        layout.addWidget(self.Company위젯)
        ...
        # Company위젯의 선택 상태 변경 시 확인 버튼 활성화 상태 업데이트
        self.Company위젯.selection_changed.connect(self.update_confirm_button)
    ...
    def get_value(self):
        return self.Company위젯.get_value()

class Company선택위젯(QWidget):
    selection_changed = pyqtSignal(bool)  # 선택 상태 변경 시그널 추가
    
    def __init__(self, parent=None, Company_list=None):
        super().__init__(parent)
        self.Company_list = Company_list or []
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        self.list_widget.addItems(self.Company_list)
        
        self.line_edit = QLineEdit()
        self.line_edit.setPlaceholderText("Company 입력 (예: 1.6T*1000*2750)")
		# # Validator 추가
        # self.validator = 생산지시서_Company_Validator(wid=self.line_edit)
    ...
    def on_select(self):
        if self.line_edit.text().strip():
            ...
            if isinstance(self.parent(), Company선택다이얼로그):
                self.parent().accept()
