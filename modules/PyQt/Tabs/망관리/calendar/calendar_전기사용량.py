...
class QCalendar_전기사용량(QCalendarWidget):
	signal_clicked = pyqtSignal(QDate)
	...
	def _is_file_exist(self, pyDate:datetime):
		date_str = pyDate.strftime('%Y-%m-%d')
		for obj in self.api_data:
			if obj.get('일자') == date_str:
				return True
		return False
	...
	def _get_event_dates(self, app_DB_data:list[dict]) -> dict:		
		...
		for obj in app_DB_data:
			if ( date_str := obj.get('일자') ) is None: continue
	...
	def slot_clicked(self):
		self._gen_hover_dlg()
		self.signal_clicked.emit(self.selectedDate() )
