...
### ui import
from modules.PyQt.Tabs.망관리.ui.Ui_search_compenet_망관리_album import Ui_Search
...
class Wid_Search_for(QWidget, Qwidget_Utils):
	...
	def init_InputDict(self) ->None:
		self.inputDict = {
			...
			# '시작일' : self.ui.dateEdit_Sijak,
			# '종료일' : self.ui.dateEdit_Jonryo,
			# 'From수량' : self.ui.spinBox_FROM,
			# 'To수량' : self.ui.spinBox_TO,
	...
	def user_defined_ui_setting(self):
		...
		self.ui.comboBox_pageSize.addItems(['15'])
	...
	def triggerConnect(self):				
		...
		self.ui.PB_DownloadAll.clicked.connect(lambda:self.signal_download.emit(self.param if hasattr(self, 'param') else '') )
