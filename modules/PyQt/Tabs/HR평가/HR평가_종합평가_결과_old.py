...
### 😀😀 user : ui...
from modules.PyQt.Tabs.HR평가.ui.Ui_tab_HR평가_종합평가_결과_old import Ui_Tab_App as Ui_Tab
...
class 종합평가_결과_old__for_Tab( QWidget, Utils_QWidget):
	def __init__(self, parent:QtWidgets.QMainWindow,   **kwargs ):
		...
		_isOk, self.평가설정_api_datas = APP.API.getlist( INFO.URL_HR평가_평가설정DB_Old+INFO.PARAM_NO_PAGE )
		if _isOk:
			self.ui.Wid_Search_for._update_config ( 평가제목_fk_list = [ f"{obj.get('id')}--{obj.get('평가제목')}" for obj in self.평가설정_api_datas] )
