...
class TableView_HR평가_종합평가_결과(My_TableView):
	def __init__(self, parent, **kwargs):
		super().__init__(parent, **kwargs)

class TableModel_HR평가_종합평가_결과(My_TableModel):
	def __init__(self, parent, **kwargs):
		super().__init__(parent, **kwargs)
	...
	def setData(self, index, value, role):
		if role == Qt.ItemDataRole.EditRole:
			...
			headerName = self.header[col] ### 0_is_submit
			api_data.update({'id': self._data[row][self.header.index(f"{str(headerName.split('_')[0])}_평가결과_id")]})
...
class Delegate_HR평가_종합평가_결과(My_Table_Delegate):
	def __init__(self, parent, **kwargs):
		super().__init__(parent, **kwargs)


TABLE_NAME = 'HR평가_종합평가_결과'

HOVER_LIST = []


class Wid_Table_for_HR평가_종합평가_결과(QWidget , Handle_Table_Menu):
	"""
	...
	def __init__(self, parent,  **kwargs ):
		super().__init__( parent, **kwargs )
		self.tableView:  TableView_HR평가_종합평가_결과
		self.table_model : TableModel_HR평가_종합평가_결과
		self.delegate : Delegate_HR평가_종합평가_결과
	...
	def slot_signal_model_data_changed(self, msg:dict) -> None:
		...
		_isOk, _json = APP.API.Send( url=INFO.URL_HR평가_평가결과_DB, dataObj=dataObj, sendData={ 'is_submit': value for _, value in send_data.items() } )
		if _isOk:
	...
	### table main 에 
	def menu__등록자_선택(self, msg:dict) -> None:
		row = msg.get('row')
		obj:dict = self.api_data[row]
		
		ids:list = [ str(id) for id in obj.get('sales_input_fks') ]
		param = f"?ids={ ','.join( ids )}&page_size=0"
		_isOK, _json = APP.API.getlist( '영업mbo/사용자등록_DB/' + param )

		if len(_json ) > 0:
			dlg = QDialog(self)
			hLayout = QVBoxLayout()
			from modules.PyQt.component.listwidget.listwidget_with_select import ListWidget_With_Select
			wid = ListWidget_With_Select( self, _data=_json)
			hLayout.addWidget(wid)
			dlg.setLayout(hLayout)
			dlg.show()
			wid.signal_selected.connect ( lambda rowNo : self.slot_등록자선택 ( dlg, obj, _json[rowNo]  ) )
	...
	def slot_select_row(self, wid:QWidget, HR평가_종합평가_결과_apiDict:dict, EL_한국정보_ID:int) :
		""" apiDict : Elevator 한국정보 Model data로 \n
			apiDict.get('id') 로 fk 사용
		"""
		_is_ok, _json = APP.API.Send( self.url, HR평가_종합평가_결과_apiDict, {'현장명_fk': EL_한국정보_ID})
		if _is_ok:
			wid.close()
			self.signal_refresh.emit()
		else:
	...
	def menu__new_row(self, msg:dict) -> None:
		"""
			copy msg.get('row') 하여 insert 함, HR평가_종합평가_결과은 일자만 복사하여 유지		
		"""
		row :int = msg.get('row')
		self.tableView : TableView_HR평가_종합평가_결과
	...
	def menu__upgrade_row(self, msg:dict) ->None:
		"""
			copy msg.get('row') 하여 insert 함, HR평가_종합평가_결과은 버젼만 default 0.01 up함		
		"""
		row :int = msg.get('row')
		self.tableView : TableView_HR평가_종합평가_결과
		model_datas:list[list] = self.tableView.model()._data
	...
	def _get_m2mField_info (self, msg:dict) ->tuple[str,str] :
		...
		match self._get_headerName(colNo):
			...
			case '완료파일수':
				m2mField = '완료file_fks'
				URL_파일_m2m  = '/HR평가_종합평가_결과/완료file-viewSet/'
				URL_파일_m2m  = getattr( self, f"URL_{m2mField}")
			case _:
				URL_파일_m2m = ''
				m2mField = ''
