...
class 성과평가TableModel(QAbstractTableModel):
    def __init__(self, data: list[dict], parent=None):
        super().__init__(parent)
        self.headers = ['과제명', '과제목표', '성과', '목표달성률', '실행기간', '가중치', '평가점수']
        self._data = data
    ...
    def insertRow(self, position: int, parent=QModelIndex()) -> bool:
        ...
        new_row = {
                ...
                '성과평가_fk': self._data[0].get('성과평가_fk'),
            }
...
class Wid_table_성과평가(QWidget):
    ...
    def set_api_datas(self, api_datas:list[dict], 평가체계_dict:dict=None):
        ...
        self.성과평가_fk = api_datas[0].get('성과평가_fk', None)
        self.data = api_datas
    ...
    def init_ui(self):
        ...
        self.add_btn = QPushButton("성과 추가")
        self.del_btn = QPushButton("성과 삭제")
        ...
        self.model = 성과평가TableModel(self.data, self)
        self.table.setModel(self.model)
        

        # Delegate 설정
        self.과제명_delegate = TextEditDelegate(self)
        self.table.setItemDelegateForColumn(0, self.과제명_delegate)
        self.과제목표_delegate = TextEditDelegate(self)
        self.table.setItemDelegateForColumn(1, self.과제목표_delegate)
        self.성과_delegate = TextEditDelegate(self)
        self.table.setItemDelegateForColumn(2, self.성과_delegate)
        self.가중치_delegate = SpinBoxDelegate(self)
        self.table.setItemDelegateForColumn(5, self.가중치_delegate)
        self.평가점수_delegate = SliderDelegate(self)
        self.table.setItemDelegateForColumn(6, self.평가점수_delegate)
        self.실행기간_delegate = TextEditDelegate(self)
        self.table.setItemDelegateForColumn(3, self.실행기간_delegate)
        self.목표달성률_delegate = TextEditDelegate(self)
        self.table.setItemDelegateForColumn(4, self.목표달성률_delegate)

        self.평가점수_delegate = SpinDoubleBoxDelegate(self)
        self.table.setItemDelegateForColumn(6, self.평가점수_delegate)
    ...
    def on_save_clicked(self):
        if hasattr(self, 'url') and self.url:
            data = {'성과평가':json.dumps(self.get_api_send_data(), ensure_ascii=False)}
            ...
            if is_ok:
                ...
                self.model.set_data(_json.get('성과평가_api_datas', []))
                self.render_summary()
...
if __name__ == '__main__':
    성과평가_api_datas= [{'id': 181, '과제명': None, '과제목표': '', '성과': '', '목표달성률': '', '실행기간': '', '가중치': 0, '평가점수': 0.0, '등록일': '2025-05-28T13:43:52.593161', '성과평가_fk': 353}]

    app = QApplication([])
    main_window = QMainWindow()
    main_window.setMinimumSize(1800, 800)
    wid_table = Wid_table_성과평가(main_window, 성과평가_api_datas)
