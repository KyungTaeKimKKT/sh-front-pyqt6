...
class 평가결과TableModel(QAbstractTableModel):
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            match colName:
                ...
                case "역량평가"|"성과평가"|"특별평가":
                    ...
                    평가점수 = rowDict.get(keyName, {}).get("평가점수", "")
                    return f"{평가점수:.2f}"
                case "피평가자_제출":
                    try:
                        keyName = f"피평가자_본인평가"
    ...
    def editorEvent(self, event, model, option, index):
        ...
        if colName in [ "역량평가", "성과평가", "특별평가"]:
            ...
            try:
                ...
                dlg.exec()
                self.table_name = self.parent().parent().table_name
                if not self.table_name:
                    raise ValueError(f"table_name 없음")
                event_bus.publish(f"{self.table_name}:request_refresh", True)
                print ('세부평가 : ','request_refresh 발생')
                return True
