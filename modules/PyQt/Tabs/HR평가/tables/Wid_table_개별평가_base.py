...
class Base_개별평가_TableModel(QAbstractTableModel):
    def __init__(self, headers: list[str], default_row_func: Callable[[], dict], data: list[dict], parent=None):
        super().__init__(parent)
        self.headers = headers
        self.default_row_func = default_row_func
        self._data = data
    ...
    def insertRow(self, position: int, parent=QModelIndex()) -> bool:
        ...
        self._data.insert(position, self.default_row_func())
        self.endInsertRows()
        return True
...
class Wid_table_특별평가(QWidget):
    ...
    def init_ui(self):
        ...
        add_btn = QPushButton("특별성과 추가")
        del_btn = QPushButton("특별성과 삭제")
        ...
        # Delegate 설정
        # self.구분_delegate = TextEditDelegate()
