...
class TableView_HR평가_역량평가(My_TableView):
	def __init__(self, parent, **kwargs):
		super().__init__(parent, **kwargs)

class TableModel_HR평가_역량평가(My_TableModel):
	def __init__(self, parent, **kwargs):
		super().__init__(parent, **kwargs)
...
class Delegate_HR평가_역량평가(My_Table_Delegate):
	def __init__(self, parent, **kwargs):
		super().__init__(parent, **kwargs)
...
TABLE_NAME = 'HR평가_역량평가'

HOVER_LIST = []


class Wid_Table_for_HR평가_역량평가(QWidget , Handle_Table_Menu):
	"""
	...
	def __init__(self, parent,  **kwargs ):
		super().__init__( parent, **kwargs )
		self.tableView:  TableView_HR평가_역량평가
		self.table_model : TableModel_HR평가_역량평가
		self.delegate : Delegate_HR평가_역량평가
	...
	def _check_table_data(self) -> tuple[int, float]:
		model_datas = self.tableView.model()._data
		col_평가점수 = self.table_header.index('평가점수')
		
		sum_평가점수 = sum( [ row[col_평가점수] for row in model_datas] )
		average_평가점수 = sum_평가점수 / len( model_datas)
		cnt_0점수 = len( [ row[col_평가점수] for row in model_datas if row[col_평가점수] == 0 ] )

		return ( cnt_0점수, average_평가점수  )
	...
	def slot_select_row(self, wid:QWidget, HR평가_역량평가_apiDict:dict, EL_한국정보_ID:int) :
		""" apiDict : Elevator 한국정보 Model data로 \n
			apiDict.get('id') 로 fk 사용
		"""
		_is_ok, _json = APP.API.Send( self.url, HR평가_역량평가_apiDict, {'현장명_fk': EL_한국정보_ID})
		if _is_ok:
			wid.close()
			self.signal_refresh.emit()
		else:
	...
	def menu__new_row(self, msg:dict) -> None:
		"""
			copy msg.get('row') 하여 insert 함, HR평가_역량평가은 일자만 복사하여 유지		
		"""
		row :int = msg.get('row')
		self.tableView : TableView_HR평가_역량평가
	...
	def menu__upgrade_row(self, msg:dict) ->None:
		"""
			copy msg.get('row') 하여 insert 함, HR평가_역량평가은 버젼만 default 0.01 up함		
		"""
		row :int = msg.get('row')
		self.tableView : TableView_HR평가_역량평가
		model_datas:list[list] = self.tableView.model()._data
	...
	def _get_m2mField_info (self, msg:dict) ->tuple[str,str] :
		...
		match self._get_headerName(colNo):
			...
			case '완료파일수':
				m2mField = '완료file_fks'
				URL_파일_m2m  = '/HR평가_역량평가/완료file-viewSet/'
				URL_파일_m2m  = getattr( self, f"URL_{m2mField}")
			case _:
				URL_파일_m2m = ''
				m2mField = ''
