...
### ui import
from modules.PyQt.Tabs.영업mbo.ui.Ui_search_compenet_사용자등록 import Ui_Search

from modules.PyQt.User.object_value import Object_Set_Value
...
class Wid_Search_for(QWidget, Qwidget_Utils):
	...
	def _update_config(self, **kwargs):
		...
		# if hasattr( self, 'year_list') :
		# 	self.ui.comboBox_Year.addItems( ['ALL'] + self.year_list)
		# 	self.ui.comboBox_Year.setEnabled(True)
		# if hasattr( self, 'month_list'):
		# 	self.ui.comboBox_Month.addItems ( ['All'] + self.month_list)
		# 	self.ui.comboBox_Month.setEnabled(True)

	def _setParam( self, param :dict):
		""" inputDict와 같은 key: value는 set 하고자 하는 value \n
			setValue이후 ok button click
		"""
		for key, value in param.items():
			Object_Set_Value( input=self.inputDict[key], value=value)
		
		self.ui.PB_Search.click()
