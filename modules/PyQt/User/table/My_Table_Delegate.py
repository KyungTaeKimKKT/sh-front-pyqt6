...
class My_Table_Delegate(QtWidgets.QItemDelegate):
	...
	def createEditor(self, parent, option, index):
		...
		if not key in self.no_Edit_cols:
			match utils.get_dataType( value ):
				...
				case _:
					widget = QLineEdit(parent)
					return self.user_defined_creatorEditor_설정( widget, key=key, value=value, parent=parent, index=index, option=option)
	...
	# def user_defined_cratorEditor(self, editor:QWidget, **kwargs) ->QtWidgets.QWidget:
	# 	return editor
	...
	def setEditorData(self, editor:QWidget, index:QModelIndex) ->None:
		...
		if not key in self.no_Edit_cols:		
			...
			### 😀😀 필요시 Hard-coding
			self.user_defined_setEditorData(editor, index, row=row,col=col,key=key, value=value)
	...
	def setModelData(self, editor, model, index):
		# model.close
		# self.model().closeEditor.emit( editor )
