...
class Search_for_tab_생지(QWidget):
	"""
	...
	def __init__(self, parent, **kwargs):
		...
		self.combo_search_items =  list(self.search_url.keys() )
	...
	def UI(self) -> None:
		...
		self.pb_form_New.clicked.connect(self.slot_pb_Search)
		self.combo_search = QComboBox()
		self.combo_search.addItems(self.combo_search_items )
		self.combo_search.setCurrentText ( self.combo_search_items[0])
		...
		self.no_data_hlayout.addWidget(self.combo_search )
	...
	def slot_pb_Search(self):		
		구분 = self.combo_search.currentText()

		self.search_msg['search'] = self.검색_lineEdit.text()
		self.search_msg['url'] =  self.search_url.get(구분)
		self.search_msg['구분'] =  구분
