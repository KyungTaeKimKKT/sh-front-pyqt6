...
# class App설정_개발자__for_stacked_Table( Custom_QStackedWidget, LazyParentAttrMixin ):
	...
	@property
	def table_config_enabled(self) -> bool:
		### config table 활성화 여부, default는 True, 상속받은곳에서 disalbe
		return True
	...
	def on_api_datas(self, api_datas:list[dict]):
		logger.debug(f"{self.__class__.__name__} : on__api_datas : {len(api_datas)} row 적용시작 ")
	...
	def init_ui(self):
		...
		if self.table_config_enabled and INFO._get_is_table_config_admin():
	...
	def create_config_table(self):
		return Wid_TableConfigMode(self)
