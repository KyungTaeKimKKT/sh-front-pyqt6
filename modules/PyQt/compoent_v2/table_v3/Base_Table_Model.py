...
class Base_Table_Model( 
					    QAbstractTableModel, 
						Base_Table_Model_Role_Mixin, 
						Base_Table_Model_Mixin,
						Mixin_Table_Config,
						):
	""" V3임 : 확장성과 유연성이 개선된 기본 테이블 모델 클래스"""
	Roles = CustomRoles
	# 시그널 정의
	
	def __init__(self, parent:QTableView=None, **kwargs):
		"""
		...
		self.wid_table = kwargs.get('wid_table', parent.parent() )
		...
		self._original_data :list[list] = []	
		self.map_id_to_rowData :dict[int, dict] = {}
	...
	def _initialize_from_kwargs(self, **kwargs):
		...
		if kwargs  :
			self.kwargs = kwargs
			if getattr(self, 'set_kwargs_to_attr', False):
	...
	def on_all_lazy_attrs_ready(self, APP_ID:Optional[int] = None, **kwargs):
		""" 실질적인 초기화 함수 : 호환성을 위해 method 유지함."""
		...
		self.appDict =  getattr(self.wid_table, "appDict", False) 
		self.table_name = getattr( self.wid_table, "table_name", False )
	...
	def set_table_config(self, table_config:dict, table_config_api_datas:list[dict]):
		""" table_config 설정 ==> 즉, 적용하여 layoutChanged.emit() 호출 """
		...
		self._header_types = self.table_config['_headers_types']
		self._headers = self.table_config['_headers']
		self._column_types = self._header_types
		self._column_widths = self.table_config['_column_widths']
		self._column_order = self.table_config['_column_order']
		self._column_hidden = self.table_config['_column_hidden']
	...
	def on_api_datas_received(self, api_datas:list[dict]):
		...
		print ( self._data )
		# if INFO.IS_DEV:
		# 	print (f"on_api_datas_received: {self._data}")
	...
	def is_all_no_edit(self):
		""" 모든 컬럼을 편집 제한 여부 """
		try: 
			values = self.lazy_attr_values.get('no_edit_columns_by_coding', [] )
			return any (['all' in values, 'All' in values, 'ALL' in values ])
		except:
			return False
	...
	def setData(self, index:QModelIndex, value, role:Qt.ItemDataRole=Qt.ItemDataRole.EditRole) -> bool:
		...
		try:
			...
			return self.setData_by_edit_mode(index, value)
		except Exception as e:
	...
	def setData_by_edit_mode(self, index:QModelIndex, value) -> bool:
		row, col = index.row(), index.column()
		db_attr_name = self.get_field_name_by_index(index)
	...
	def exist_field(self, db_attr_name:str) -> bool:
		return db_attr_name in self.table_config['_headers']
	...
	def get_choices(self, index:QModelIndex) -> list:
		""" lazy_attrs 에 정의된 "model" : "choice_field" 에 정의된 db 속성이름으로,
		    drf serializer 에서 정의된 choices 를 반환함.
			따라서, delegate에서 호출함.
		"""
		attr_name = self.get_attr_name_by_index( index)
		choice_field:dict = self.lazy_attrs.get('model', {}).get('choice_field', {} )
		if attr_name in choice_field:
			rowDict_field = choice_field[attr_name]
			return self._data[index.row()][rowDict_field]
		else:
			return []
	...
	def on_api_send_By_Row(self, send_type:str='formdata'):
		if send_type == 'formdata':
			if 'FileField' in list(self.table_config['_column_types'].values()):
				file_field_name = self.get_file_field_name()
				if file_field_name:
					return self.on_api_send_By_Row_with_file(file_field_name=file_field_name)
				else:
	...
	def get_file_field_name(self) -> list[str]:

		for col_no, col_name in enumerate(self.get_headers()):
			if self.table_config['_column_types'][col_name] == 'FileField':
				return col_name
		return None
