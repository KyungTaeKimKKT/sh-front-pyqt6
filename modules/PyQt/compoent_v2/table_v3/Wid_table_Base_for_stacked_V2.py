...
from modules.PyQt.compoent_v2.table_v3.Base_Table_View import Base_Table_View
from modules.PyQt.compoent_v2.table_v3.Base_Table_Model import Base_Table_Model
from modules.PyQt.compoent_v2.table_v3.Base_Delegate import Base_Delegate
...
class Wid_table_Base_V2( 
    ...
    def __init__(self, parent:QWidget, **kwargs):
        ...
        self.model : Base_Table_Model = None
        self.view : Base_Table_View = None
        self.delegate : Base_Delegate = None
        # self.setup_table()
        # self.init_by_parent()
        ...
        if  not ('disable_lazy_attr' in self.kwargs and self.kwargs['disable_lazy_attr']):
            self.run_lazy_attr()

    def on_all_lazy_attrs_ready(self):		
        if INFO.IS_DEV:
            print (f"{self.__class__.__name__} :on_all_lazy_attrs_ready: {self.lazy_attrs}")
        try:
    ...
    def _parse_objectName(self, objectName:str) -> dict|str:
        if 'json' in objectName:
            return json.loads(objectName.replace('json:', ''))
        else:
            return objectName
    ...
    def on_file_download(self):
        try:
            ...
            if objectName :
                if 'json' in objectName:
                    object_dict = json.loads(objectName.replace('json:', ''))
                    print(f"object_dict: {object_dict}")
                    if object_dict and "type" in object_dict and "db_attr_name" in object_dict:
                        file_path = self.selected_dataObj[object_dict['db_attr_name']]
        ...
        try:
            ...
            if objectName:
                if 'json' in objectName:
                    ...
                    print(f"object_dict: {object_dict}")
                    if object_dict and "type" in object_dict and "db_attr_name" in object_dict:
                        db_attr_name = object_dict['db_attr_name']
                        if not self.model.exist_field(db_attr_name):
                            Utils.generate_QMsg_critical(self, title='파일 업로드 오류', text=f'파일 업로드 오류<br>db_attr_name: {db_attr_name} 이 없읍니다.')
                            return 
                        
                        _type:str = object_dict['type']
                        if _type == 'single':
                            file = FileOpenSingle(self)
                            file_path = file.open_file_dialog()
                        elif _type == 'multi' or _type == 'multi_file' or _type == 'multiple':
                            logger.error(f"on_file_upload 오류: multiple file upload는 지원하지 않읍니다. {objectName}")
                            return 
                        else:
                    ...
                    db_attr_name = objectName
                    file_path = FileOpenSingle(self)

                if file_path:
                    self.selected_dataObj[db_attr_name] = file_path
                    self.model.layoutChanged.emit()
                else:
                    Utils.generate_QMsg_critical(self, title='파일 업로드 오류', text=f'파일 업로드 오류<br>objectName: {objectName}')
            else:
